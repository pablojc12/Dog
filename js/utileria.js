...
/**
 * Funcion para verificar si el boleano es true esto retornara true,
 * de lo cantrario mandara un mensaje y enfocara el elemento deceado
 * @param {boolean} booleano
 * @param {text} idElement
 * @param {text} mensaje
 * @returns {Boolean}
 */
function booleanoCMensajeYFoco(booleano, idElement, mensaje)
{
    if(!booleano)
    {
        controlDosMensajes(mensaje);
...
/**
 * Funcion para comparar dos fechas, cuando la fechaUno es mayor que la fechaDos retornara un false
 * en caso contrario retornara un true
 * EL FORMATO DE FECHA REQUERIDO ES AAAA-MM-DD
 * @param {text} fechaUno
 * @param {text} fechaDos
 * @returns {Boolean}
 */
function comparaDosFechas(fechaUno, fechaDos)
{
    var elementS = fechaUno.split("-");
    var elementR = fechaDos.split("-");
 ...
 * 
 */
function ajaxLoad(objetoJSON, boolean) 
{
    console.log(boolean);
    $.ajax({
        url: server,
